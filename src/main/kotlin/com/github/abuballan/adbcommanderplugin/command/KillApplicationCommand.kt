...
class KillApplicationCommand(private val adbCommandPrams: AdbCommandPrams) : AdbCommand<Unit>(adbCommandPrams) {
    ...
    private fun runCommandIfAppInstalled(
        ...
        return when (val shellReceiverResult = device.execute("am force-stop $packageName", shellReceiver)) {
            ...
            when (result) {
                is AdbCommandResult.Success -> {
                    project.showNotification(
                        ...
                        "Adb Commander",
                        "<b>$packageId</b> forced stop on ${result.deviceName}"
                    )
                }
                is AdbCommandResult.Error -> {
                    project.showNotification(
